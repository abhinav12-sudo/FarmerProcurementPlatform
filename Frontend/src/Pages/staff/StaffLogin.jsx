...
export default function StaffLogin() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-slate-200 to-slate-300 flex flex-col justify-center items-center p-4 sm:p-6">
      <div className="w-full max-w-md mb-4 flex items-center justify-between">
        <button
          onClick={() => navigate('/')}
          className="inline-flex items-center gap-1.5 text-sm font-medium text-emerald-900 hover:text-emerald-950 transition cursor-pointer"
        >
        ...
        <span className="text-xs font-semibold px-2.5 py-1 bg-slate-200 text-slate-800 rounded-full flex items-center gap-1">
          <ShieldCheck className="w-3.5 h-3.5 text-emerald-700" /> Staff Portal
        </span>
      </div>
        ...
        <div className="flex border-b border-gray-100 bg-gray-50/70 p-1.5 gap-1.5 text-xs font-medium text-gray-600">
          <button
            ...
            className={`flex-1 py-2 text-center rounded-xl transition cursor-pointer font-semibold ${
              !isRegister ? 'bg-white text-slate-900 shadow-sm' : 'hover:text-gray-900'
            }`}
          >
            Officer Sign In
          </button>
          ...
          <form onSubmit={handleRegisterSubmit} className="p-6 space-y-3.5">
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 bg-emerald-700 hover:bg-emerald-800 text-white font-medium py-2.5 px-4 rounded-xl shadow-md transition cursor-pointer disabled:opacity-70 text-sm"
              >
                {loading ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
